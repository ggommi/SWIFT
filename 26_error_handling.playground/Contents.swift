...
class VendingMachine {
    ...
    func vend(numberOfItems numberOfItemsToVend: Int) throws -> String {
        ...
        guard numberOfItemsToVend > 0 else {
            throw VendingMachineError.invalidInput //return이 아닌 throw가 나온다. 오류를 던지며 함수를 끝냄
        }
        
        // 구매하려는 수량보다 미리 넣어둔 돈이 적으면 오류를 던진다.
        guard numberOfItemsToVend * itemPrice <= deposited else {
            let moneyNeed: Int
            moneyNeed = numberOfItemsToVend * itemPrice - deposited
            
            throw VendingMachineError.insufficientFunds(moneyNeeded: moneyNeed)
        }
...
} catch /* (let error) // error가 자동인자로 넘어온다 */  {
    switch error {
...
//try!의 경우
//오류가 발생하지 않을 것이라는 강력한 확신을 가질때
//try!를 사용하면 정상동작 후에 바로 결과값을 돌려받는다
//오류가 발생하면 런타임 오류가 발생하여
//애플리케이션 동작이 중지된다.

//result = try! machine.vend(numberOfItems: 1) //이 과정에서 옵셔날타입이 아닌 정상타입을 제공받는다.
result // 1개 제공함
